...
func LoadIPAMConfig(bytes []byte, args string) (*IPAMConfig, string, error) {
	...
	if n.EtcdEndpoints == nil {
                return nil, "", fmt.Errorf(`""etcdEndpoints" is must"`)
        }

        if n.EtcdPrefix == "" {
                return nil, "", fmt.Errorf(`""etcdPrefix" is must"`)
        }
	...
	n.IPAM.Name = n.Name
	n.IPAM.EtcdEndpoints = n.EtcdEndpoints
	n.IPAM.EtcdPrefix = n.EtcdPrefix
